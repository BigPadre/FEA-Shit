...
def Two_D_6DOF_beam_stiffness_mat_Global_peice(num_beam_el,L_list, E_list, I_list,theta_list,A_list):
...
def assemble_global_stiffness_matrix(k_global_piece_list, con_mat, Tot_DOF, N_DOF_element):
    ...
    for i, k_global_piece in enumerate(k_global_piece_list):#i assume that enumerate gives the indexed matrix of the element in the piece list?
        current_nodes = con_mat[i]  # Get the nodes for the current element (3DOF need to be assigned indexes per el)
        dof_indices = [current_nodes[0] * N_DOF_element, current_nodes[0] * N_DOF_element + 1,current_nodes[0] * N_DOF_element + 2,
                       current_nodes[1] * N_DOF_element, current_nodes[1] * N_DOF_element + 1, current_nodes[1] * N_DOF_element + 2]
